...
import { Search, MapPin, FileDown } from "lucide-react";
...
export default function EmployerCandidateList() {
  ...
  const handleViewReport = (
    ...
    link.href = "/skillkwiz-report.pdf";
    link.download = `skillkwiz-report-${candidateName.toLowerCase().replace(/\s+/g, "-")}-${candidateId}.pdf`;
          ...
          {candidates.map((candidate) => (
            <div key={candidate.id} className="bg-[#4a63b3]/80 rounded-lg p-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => handleViewReport(candidate.name, candidate.id)}
                  className="bg-[#00bcd4] text-white px-4 py-2 rounded-lg hover:bg-[#00a5bb] transition-colors duration-200 flex items-center gap-2 font-medium text-sm shadow-md hover:shadow-lg"
                >
